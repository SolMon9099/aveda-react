...
import { createOrUpdateActivity } from "src/redux/slices/processDetail";
...
const useActivitiesList = () => {
    ...
    const { activities } = useSelector((state) => state.processDetail)
    ...
    const TAGS_OPTIONS = [
      {value: '1', label: 'LABEL 1', color: 'error'},
      {value: '2', label: 'LABEL 2', color: 'success'}
    ]
