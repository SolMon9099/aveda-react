...
export default function NavItem({ item, depth, active, open, ...other }: Props) {
  const { translate } = useLocales();
  const { isAuthenticated } = useAuth()

  const { title, icon, info, children, disabled, caption, needAuth } = item;

  const show = (needAuth && isAuthenticated) || (!needAuth);
