...
export function getWhoLiked(postId: string) {
    return async () => {
        try {
            const response = await api.get(`/post/getWhoLiked/${postId}`);
...
export function likePost(userId: string, postId: string) {
    return async () => {
        try {
            await api.post(`/post/like`, {userId, postId});
        } catch (error) {
            dispatch(slice.actions.hasError(error));
        }
    };
...
export function savePost(userId: string, postId: string) {
    return async () => {
        try {
            await api.post(`/post/savePost`, {userId, postId});
        } catch (error) {
            dispatch(slice.actions.hasError(error));
        }
    };
...
export function deletePost(postId: string) {
    return async () => {
        try {
            await api.post(`/post/deletePost`, {postId});
        } catch (error) {
            dispatch(slice.actions.hasError(error));
        }
    };
...
export function commentPost(userId: string, postId: string, body: string, images: File[], links: string[], parentComment?: string) {
    return async () => {
        try {
            ...
            await api.post(`/post/comment/create`, {author: userId, post: postId, body, parentComment, attachments});
        } catch (error) {
            dispatch(slice.actions.hasError(error));
        }
    };
...
export function likeComment(userId: string, commentId: string) {
    return async () => {
        try {
            await api.post(`/comment/like`, {userId, commentId});
        } catch (error) {
            dispatch(slice.actions.hasError(error));
        }
    };
